...
=======
- **🔐 Biometric Authentication**: DeepFace AI with Facenet model for facial recognition (0.40 cosine distance threshold)
- **⛓️ Blockchain Voting**: Immutable vote storage on Ethereum (Ganache)
- **🎯 Liveness Detection**: Blink detection + frame variation analysis to prevent photo/video spoofing
...
- **🔍 Voter Search**: Search by Voter ID or Aadhaar with auto-formatting
...
│   └── VotingContract.json # Contract ABI (generated on deploy)
├── public/                 # User guide markdown files
...
6. **Liveness Challenge** (1 challenge, 10 seconds total):
   - First 5 seconds: Blink naturally (liveness detection)
   - Last 5 seconds: Stay still (face matching capture)
   - **During challenge**: System captures 15 frames over 3 seconds for liveness detection
7. **Image Capture**: Main authentication image captured 1 second into the challenge
...
│   └── api.ts              # API service layer
├── backend/
│   ├── app.py              # Flask server (1,821 lines)
│   ├── liveness_detector.py # Liveness detection module
│   ├── setup_db.py         # Database setup script
│   ├── deploy_contract.py  # Contract deployment script
│   ├── schema.sql          # Database schema
...
>>>>>>> 3933b87 (Added screenshots to README)
